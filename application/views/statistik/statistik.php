<script type="text/javascript">
	...
	document.addEventListener("DOMContentLoaded", function () {
		...
		let yyyyStart = kemarin.getFullYear();
		let mmStart = String(kemarin.getMonth() + 1).padStart(2, '0');
		let ddStart = String(kemarin.getDate()).padStart(2, '0');

		let yyyyEnd = sekarang.getFullYear();
		let mmEnd = String(sekarang.getMonth() + 1).padStart(2, '0');
		let ddEnd = String(sekarang.getDate()).padStart(2, '0');

		document.getElementById('dtStart').value = yyyyStart + '-' + mmStart + '-' + ddStart;
		document.getElementById('dtEnd').value   = yyyyEnd + '-' + mmEnd + '-' + ddEnd;
    });
</script>
...
<div class="container-fluid">
	<div class="mb-4">
		...
		<a href="#modal-pdf" class="btn btn-light shadow-sm mr-2" data-toggle="modal">
			<i class="fas fa-file-pdf fa-sm text-danger"></i> Generate Laporan PDF
		</a>
		<!-- 2026.01.05 -->
		<a href="#modal-newExcel" class="btn btn-light shadow-sm" data-toggle="modal">
			<i class="fas fa-file-excel fa-sm text-success"></i> Download Laporan
		</a>
	</div>
...
<div id="modal-newExcel" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="m-0 font-weight-bold text-gray-800">Download Laporan Excel</h5>
			</div>
			<div class="modal-body">
				<form id="form-validation" action="<?= site_url('statistik/downloadReport') ?>" method="POST" enctype="multipart/form-data">
					<div class="form-group">
					...
					<div class="form-group">
						<label>Dari Tanggal</label>
						<input type="date" class="form-control" name="dtStart" id="dtStart" placeholder="Pilih Tanggal" autocomplete="off" required>
					</div>

					<div class="form-group">
						<label>Sampai Tanggal</label>
						<input type="date" class="form-control" name="dtEnd" id="dtEnd" placeholder="Pilih Tanggal" autocomplete="off" required>
					</div>
					<button type="submit" class="btn btn-primary" formtarget="_blank">Download</button>
					<button type="button" class="btn btn-danger" class="close" data-dismiss="modal">Cancel</button>
				</form>
