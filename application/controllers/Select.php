...
class Select extends CI_Controller
{
	...
	public function select_job()
	{
		...
		$data['dataassigment'] = $this->db->query("SELECT A.progress, A.status, A.id_ticket, A.tanggal, B.nama_sub_kategori, C.nama_kategori, D.nama, D.email FROM ticket A 
		...
		$row = $this->db->query("SELECT A.nama, A.email FROM pegawai A LEFT JOIN user B ON A.nik = B.username WHERE B.username = '$id_teknisi'")->row();
		if ($id_teknisi == "") {
			$data['nama']  	= "";
			$data['email']  = "";
		} else {
	...
	public function select_prioritas()
	{
		$id_prioritas = $this->input->post('id_prioritas');
		$row = $this->db->query("SELECT waktu_respon FROM prioritas WHERE id_prioritas = '$id_prioritas'")->row();
		if ($id_prioritas == "") {
			$data['waktu_respon']  	= "";
		} else {
		...
		//Load template
		$this->load->view('combo/select_prioritas', $data);
	}
	...
	public function select_sub()
	{
		...
		if ($id_kategori == "") {
			//Dropdown pilih sub kategori, menggunakan model (dropdown_sub_kategori), nama sub kategori ditampung pada 'dd_sub_kategori', data yang akan di simpan adalah id_sub_kategori dan akan ditampung pada 'id_sub_kategori'
			...
			//Dropdown pilih sub kategori, menggunakan model (dropdown_sub_kategori) mengikuti id_kategori yang dipilih, nama sub kategori ditampung pada 'dd_sub_kategori', data yang akan di simpan adalah id_sub_kategori dan akan ditampung pada 'id_sub_kategori'
			$data['dd_sub_kategori'] = $this->model->dropdown_sub_kategori($id_kategori);
			$data['id_sub_kategori'] = "";
		}
	...
	public function select_subdept()
	{
		...
		if ($id_departemen == "") {
			//Dropdown pilih sub departemen, menggunakan model (dropdown_bagian_departemen), nama sub departemen ditampung pada 'dd_bagian_departemen', data yang akan di simpan adalah id_bagian_departemen dan akan ditampung pada 'id_bagian_departemen'
			...
			//Dropdown pilih sub departemen, menggunakan model (dropdown_bagian_departemen) mengikuti id_dept yang dipilih, nama sub departemen ditampung pada 'dd_bagian_departemen', data yang akan di simpan adalah id_bagian_departemen dan akan ditampung pada 'id_bagian_departemen'
			$data['dd_bagian_departemen'] = $this->model->dropdown_bagian_departemen($id_departemen);
			$data['id_bagian_departemen'] = "";
		}
	...
	public function select_email()
	{
		...
		$row 			= $this->db->query("SELECT * FROM pegawai WHERE nik = '$id_pegawai'")->row();
		$data['nama']  	= $row->nama;
		$data['email']  = $row->email;
	...
	public function select_tahun()
	{
		...
		$data['stat_prio_tahun'] = $this->db->query("SELECT A.id_prioritas, B.nama_prioritas, B.warna, COUNT(*) AS jumprioritas FROM ticket A 
		LEFT JOIN prioritas B ON B.id_prioritas = A.id_prioritas 
		WHERE YEAR(A.tanggal) = '$id_tahun' AND A.status NOT IN (0)
		GROUP BY A.id_prioritas ORDER BY A.id_prioritas ASC")->result();
		...
		if ($id_tahun == "") {
			...
			//Bagian ini adalah kondisi dimana admin memilih salah satu departemen, maka sub departemen dari departemen yang dipilih akan ditampilkan
			//Dropdown pilih sub departemen, menggunakan model (dropdown_bagian_departemen) mengikuti id_dept yang dipilih, nama sub departemen ditampung pada 'dd_bagian_departemen', data yang akan di simpan adalah id_bagian_departemen dan akan ditampung pada 'id_bagian_departemen'
			$data['dd_bulan'] = $this->model->pilih_bulan($id_tahun);
			$data['id_bulan'] = "";
		}
	...
	public function select_bulan()
	{
		...
		$data['stat_prio_bulan'] = $this->db->query("SELECT A.id_prioritas, B.nama_prioritas, B.warna, COUNT(*) AS jumprioritas FROM ticket A 
		LEFT JOIN prioritas B ON B.id_prioritas = A.id_prioritas 
		WHERE DATE_FORMAT(A.tanggal, '%Y/ %M')='$id_bulan' AND A.status NOT IN (0)
		GROUP BY A.id_prioritas ORDER BY A.id_prioritas ASC")->result();
	...
	public function select_vehicle()
	{
		...
		if ($id_dept == "") {
			...
			$data['dd_vehicle'] = $this->model->dropdown_vehicle($id_dept);
			$data['id_vehicle'] = "";
		}

		//Load template
		$this->load->view('combo/select_vehicle', $data);
	}
