...
class Main_model extends CI_Model
{
  ...
  public function getkodeticketnew()
  {
    ...
    $random = strtoupper(random_string('alnum', 4)); // 3 digit alfanumerik acak
  ...
  function dropdown_spv_tech()
  {
    ...
    $query = $this->db->query("SELECT 'SPVU' AS username, 'Supervisor Maintenance' AS nama FROM dual
                              UNION ALL
                              SELECT 'SPVM' AS username, 'Dies, Workshop, & Facility Engineer' AS nama FROM dual");
  ...
  public function dropdown_target()
  {
    ...
    $sql = "SELECT * FROM departemen d WHERE id_dept IN ('12','8','11') ORDER BY 1";
    $query = $this->db->query($sql);
...
public function approveIncidentMgr($id)
  ...
  public function maintenance($id)
  {
    $query = $this->db->query("SELECT wom.wo_id,
                                      ...
                                      wom.status,
                                      CONCAT(
                                          FLOOR(TIMESTAMPDIFF(SECOND, wom.req_time, COALESCE(wom.checker_time, NOW())) / 3600), ':',
                                          LPAD(FLOOR((TIMESTAMPDIFF(SECOND, wom.req_time, COALESCE(wom.checker_time, NOW())) % 3600) / 60), 2, '0'), ':',
                                          LPAD(TIMESTAMPDIFF(SECOND, wom.req_time, COALESCE(wom.checker_time, NOW())) % 60, 2, '0')
                                      ) AS respone_time,
                                      CONCAT(
                                          FLOOR(TIMESTAMPDIFF(SECOND, wom.req_time, COALESCE(wom.mtc_time, NOW())) / 3600), ':',
                                          LPAD(FLOOR((TIMESTAMPDIFF(SECOND, wom.req_time, COALESCE(wom.mtc_time, NOW())) % 3600) / 60), 2, '0'), ':',
                                          LPAD(TIMESTAMPDIFF(SECOND, wom.req_time, COALESCE(wom.mtc_time, NOW())) % 60, 2, '0')
                                      ) AS down_time
                            FROM 
