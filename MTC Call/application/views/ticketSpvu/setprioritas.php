...
<script>
	$(document).ready(function(){
		$('#id_teknisi').on('change', function(){
		const selectedId = $(this).val();
		console.log(selectedId);
		const baseUrl = "<?= site_url('ticket_spvu/approve/' . $detail['id_ticket']) ?>";
		...
		<div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
			<div class="card shadow mb-4">
				<div class="card-body">
					...
					<form id="myForm" method="post" action="<?= site_url('ticket_spvu/approve/' . $detail['id_ticket']) ?>" enctype="multipart/form-data">
						<div class="form-group">
							<label class="mb-2 font-weight-bold text-primary">Note From Manager</label>
							<textarea id="notes" class="form-control <?= (form_error('notes') ? 'is-invalid' : '') ?>" name="notes" readonly><?= $detail['notes'] ?></textarea>
						...
						<button type="button" class="btn btn-danger" onclick="window.location='<?= site_url('ticket_spvu/list_tugas_spvu') ?>'">Batal</button>
					</form>
				</div>
			</div>
		</div>
