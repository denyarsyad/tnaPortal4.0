<script language="javascript" type="text/javascript">
	$(document).ready(function() {
		$("#id_prioritas").change(function() {
			...
			$.ajax({
				type: "POST",
				url: "<?= site_url('select/select_prioritas') ?>",
				data: data,
...
<script>
	$(document).ready(function(){
		$('#id_spv_tech').on('change', function(){
		const selectedId = $(this).val();
		const baseUrl = "<?= site_url('ticket_mgr/approveMgr/' . $detail['id_ticket']) ?>";
		$('#myForm').attr('action', baseUrl + "/" + selectedId);
		});
	});
</script>
...
<div class="container-fluid">
	...
	<ul class="nav nav-tabs" id="myTab" role="tablist">
		<li class="nav-item" role="presentation">
			<a class="nav-link active" id="home-tab" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-selected="true">Assign To</a>
		</li>
		<li class="nav-item" role="presentation">
			<a class="nav-link" id="detail-tab" data-toggle="tab" href="#detail" role="tab" aria-controls="detail" aria-selected="false">Detail Work Order</a>
		</li>
	</ul>
		...
		<div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
			<div class="card shadow mb-4">
				<div class="card-body">
					...
					<form id="myForm" method="post" action="<?= site_url('ticket_mgr/approveMgr/' . $detail['id_ticket']) ?>" enctype="multipart/form-data">
						...
						<div class="form-group">
							<label class="mb-2 font-weight-bold text-primary">Note</label>
							<textarea name="notes" placeholder="" class="form-control <?= (form_error('notes') ? "is-invalid" : "") ?>" rows="2"><?= set_value('notes'); ?></textarea>
							<div class="invalid-feedback">
								<?= form_error('notes'); ?>
							</div>
						</div>
