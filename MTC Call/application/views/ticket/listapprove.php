		...
		<div class="card-body">
				...
				<table class="table table-striped" id="dataTable" width="100%" cellspacing="0">
					...
					<tbody>
						...
						foreach ($approve as $row) { ?>
							<tr>
								<td><?= $no ?></td>
								<td><a href="<?= site_url('ticket/detail_approve/' . $row->id_ticket) ?>" title="Detail Work Order <?= $row->id_ticket ?>" class="font-weight-bold"><?= $row->id_ticket ?></a></td>
								<td><?= $row->tanggal ?></td>
			...
			<div class="alert alert-warning text-dark" role="alert">
				...
				&bull; Untuk Approve Pilih <i class="fas fa-check-circle text-success"></i> lalu klik Terima untuk memilih Prioritas dan menugaskan Teknisi.<br/>
					&bull; Untuk Reject Pilih <i class="fas fa-times-circle text-danger"></i> lalu klik Tolak untuk mengarahkan ke form Alasan Tolak Work Order.
				</p>
			</div>
		</div>
	</div>
...
<script type="text/javascript">
	...
	$('.aprove').on('click', function(e) {
		...
		Swal.fire({
			...
			text: "Work Order ini akan diterima (Approve)",
			icon: 'info',
	...
	$('.reject').on('click', function(e) {
		...
		Swal.fire({
			title: 'Apa kamu yakin?',
			text: "Work Order ini akan ditolak",
			icon: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#3085d6',
			cancelButtonColor: '#d33',
