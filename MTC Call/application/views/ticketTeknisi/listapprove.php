		...
		<div class="card-body">
			...
			<div class="alert alert-warning text-dark" role="alert">
				...
				<p class="mb-0" style="font-size: 14px;">
					Silahkan ke menu <code>Dashboard -> Daftar Tugas</code> untuk update progress Work Order.
				</p>
			</div>
		</div>
	</div>
...
<script type="text/javascript">
	...
	if (flashData) {
		Swal.fire(
			'Sukses!',
			'Work Order dalam status ' + flashData,
			'success'
		)
	}
	...
	if (flashData1) {
		Swal.fire(
			'Sukses!',
			'Work Order dalam status ' + flashData1,
			'success'
		)
	}
	...
	$('.process').on('click', function(e) {
		...
		Swal.fire({
			...
			text: "Work Order ini akan diproses",
			icon: 'info',
	...
	$('.pending').on('click', function(e) {
		...
		Swal.fire({
			title: 'Apa kamu yakin?',
			text: "Work Order ini akan dipending",
			icon: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#3085d6',
			cancelButtonColor: '#d33',
