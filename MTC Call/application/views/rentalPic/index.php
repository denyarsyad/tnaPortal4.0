					...
					<tbody>
						...
						foreach ($rental as $row) { ?>
							<tr>
								<td><?= $no ?></td>
								<?php if ($row->status == "6") { ?>
									<td><a href="<?= site_url('rental_pic/finish/' . $row->id_req) ?>" class="font-weight-bold" title="Detail"><?= $row->id_req ?></a></td>
								<?php } else {  ?>
									<td><a href="<?= site_url('rental_pic/approve/' . $row->id_req) ?>" class="font-weight-bold" title="Detail"><?= $row->id_req ?></a></td>
								<?php } ?>
								<td><?= $row->req_date ?></td>
								<td><?= $row->end_date ?></td>
								<td><?= $row->nama ?></td>
								...
								<?php if ($row->status == "6") { ?>
									<td>
										<a href="<?= site_url('rental_pic/finish/' . $row->id_req) ?>" class="btn btn-primary btn-circle btn-sm" title="Detail">
										<i class="fas fa-search"></i></a>
									</td>
									
								<?php } else {  ?>
									<td>
										<a href="<?= site_url('rental_pic/approve/' . $row->id_req) ?>" class="btn btn-primary btn-circle btn-sm" title="Detail">
