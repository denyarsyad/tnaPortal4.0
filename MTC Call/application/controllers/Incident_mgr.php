...
class Incident_mgr extends CI_Controller
{
	...
	public function index()
	{
		...
		//if ($this->session->userdata('level') == "MGR") {
			...
			$data['body']     = "incidentMgr/listIncident";
			...
			$data['ticket'] = $this->model->mgrIncident($id_dept)->result();
	...
	public function buat()
	{
		...
		//if ($this->session->userdata('level') == "MGR") {
			...
			$data['navbar']   = "navbar";
			$data['sidebar']  = "sidebar";
			$data['body']     = "incidentMgr/buatIncident";
			...
			if (!$this->upload->do_upload('path_photo')) {
				...
				if ($_FILES['path_photo']['error'] != 4) {
					...
					$data = array(
						...
						//'status'    			=> 'R',
						'status'    			=> 'S', //MGR by pass to PIC Incident
						'id_action'				=> $id_user,
						'date_action'			=> $date,
	...
	public function detail($id)
	{
		...
		//if ($this->session->userdata('level') == "MGR") {
			...
			$data['detail'] = $this->model->detail_incident_mgr($id)->row_array();
	...
	public function approveMgr($id)
    {
		...
		//if ($this->session->userdata('level') == "MGR") {
			...
			$this->model->approveIncidentMgr($id);
    ...
    public function detail_reject($id)
    {
         $level = ["MGR", "MGRD"];
            ...
            $data['navbar']   = "navbar";
            $data['sidebar']  = "sidebar";
            $data['body']     = "incidentMgr/detailreject";
